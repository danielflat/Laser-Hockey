...
axes[0].plot(x_axis, mean_total_losses, color=COLOR_MAP[0])
axes[0].fill_between(x_axis, mean_total_losses - std_total_losses,
...
# Random
axes[1].plot(x_axis, random_agent_win_rate_mean, color=COLOR_MAP[0], label='Random Agent')
axes[1].fill_between(x_axis, random_agent_win_rate_mean - random_agent_win_rate_std,
                     random_agent_win_rate_mean + random_agent_win_rate_std, color=COLOR_MAP[0], alpha=0.2)
# Weak
axes[1].plot(x_axis, weak_agent_win_rate_mean, color=COLOR_MAP[1], label='Weak Agent')
axes[1].fill_between(x_axis, weak_agent_win_rate_mean - weak_agent_win_rate_std,
                     weak_agent_win_rate_mean + weak_agent_win_rate_std, color=COLOR_MAP[1], alpha=0.2)
# Strong
axes[1].plot(x_axis, strong_agent_win_rate_mean, color=COLOR_MAP[2], label='Strong Agent')
axes[1].fill_between(x_axis, strong_agent_win_rate_mean - strong_agent_win_rate_std,
                     strong_agent_win_rate_mean + strong_agent_win_rate_std, color=COLOR_MAP[2], alpha=0.2)
# DDPG
axes[1].plot(x_axis, ddpg_agent_win_rate_mean, color=COLOR_MAP[3], label='DDPG Agent')
axes[1].fill_between(x_axis, ddpg_agent_win_rate_mean - ddpg_agent_win_rate_std,
                     ddpg_agent_win_rate_mean + ddpg_agent_win_rate_std, color=COLOR_MAP[3], alpha=0.2)
axes[1].set_xlabel('Episodes')
axes[1].set_ylabel('Win Rate')
axes[1].set_title('Win Rate over Episodes')
axes[1].set_ylim(0, 1)
axes[1].axhline(y=0.57, linestyle='--', color=COLOR_MAP[0], linewidth=2, label="Random Agent (after training)")
axes[1].axhline(y=0.55, linestyle='--', color=COLOR_MAP[1], linewidth=2, label="Weak Agent (after training)")
axes[1].axhline(y=0.47, linestyle='--', color=COLOR_MAP[2], linewidth=2, label="Strong Agent (after training)")
axes[1].axhline(y=0.64, linestyle='--', color=COLOR_MAP[3], linewidth=2, label="DDPG Agent (after training)")
axes[1].legend()

print("ax 02 done")

# Save individual plots
fig.savefig("eval_tdmpc2_hockey_training1.pdf")
# axes[0].figure.savefig("eval_tdmpc2_hockey_training_episode_duration1.pdf")
axes[0].figure.savefig("eval_tdmpc2_hockey_training_episode_loss1.pdf")
axes[1].figure.savefig("eval_tdmpc2_hockey_training_episode_win_rates1.pdf")
