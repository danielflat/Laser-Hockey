...
PENDULUM_TRAINING_STEPS = 3  # The number of training steps for the pendulum environment
LUNAR_LANDER_TRAINING_STEPS = 3  # The number of training steps for the pendulum environment
...
def eval_tdmpc2_other_env():
    ...
    pendulum_reward_mean = np.array(all_pendulum_rewards).mean(0)
    pendulum_reward_std = np.array(all_pendulum_rewards).std(0, ddof=1)

    lunar_lander_x_axis = np.arange(1, LUNAR_LANDER_TRAINING_STEPS + 1)
    lunar_lander_reward_mean = np.array(all_lunar_lander_rewards).mean(0)
    lunar_lander_reward_std = np.array(all_lunar_lander_rewards).std(0, ddof=1)
